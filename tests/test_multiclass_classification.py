...
# Summary criteria
expected_results_summary_criteria = pd.read_csv('../tests/csv_multiclass_classification/summary_criteria.csv')

# Summary objects
expected_results_summary_objects = pd.read_csv('../tests/csv_multiclass_classification/summary_objects.csv')

# Object probabilities
expected_results_object_probabilities = pd.read_csv('../tests/csv_multiclass_classification/object_probabilities.csv')

# Weights
expected_results_weights = pd.read_csv('../tests/csv_multiclass_classification/weights.csv')

# load the data
train_data = pd.read_csv("../examples/data/wine_train.csv").iloc[:, 1:]
train_labels = pd.read_csv("../examples/data/wine_train_labels.csv").iloc[:, 1].values

# Define a range of regularisation parameters C for elastic net. A minimum of at least one value is required.
my_C_params = [0.001, 0.01, 0.1, 1]

# Define a range of l1-ratios for elastic net.  A minimum of at least one value is required.
my_l1_ratios = [0, 0.25, 0.5, 0.75, 1]
...
analysis.train()
selected_features = analysis.select_features(tau_1_cutoff=0.33, tau_2_cutoff=0.33, tau_3_cutoff=0.975)
